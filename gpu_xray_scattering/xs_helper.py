...
def xray_scatter(coord, ele, vdW, q, 
                 use_oa=0, num_q_raster=1024, rho=0.334):
    ...
    c_vdW = (c_float * num_atom)(*vdW)
    c_q = (c_float * num_q)(*q)
    c_S_calc = (c_float * num_q)()

    xs_calc(c_int(num_atom), c_coord, c_ele, c_vdW, c_int(num_q), c_q, c_S_calc, 
            c_int(use_oa), c_int(num_q_raster), c_float(rho))
